...
// @version      0.3.2
// @description  Read-only cross-site DOM investigator with normalized captures, iPad/Safari picking, chat-friendly file export, and built-in help.
// @match        http://*/*
// @match        https://*/*
...
(() => {
  ...
  const CHAT_EXPORT_VERSION = 'gfi.chat-export/1';
  ...
  function eventBindings(el) {
    ...
    for (const a of [...el.attributes]) {
      if (/^on/i.test(a.name) || a.name === 'data-bind' || /^data-(action|event|click|command)/i.test(a.name)) {
        out[a.name] = a.value.slice(0, 1500);
      }
    }
    return out;
  }
  ...
  function koContextSummary(el) {
    ...
    for (let depth = 0; node && depth < 12; depth++, node = node.parentElement) {
      let ctx;
      try { ctx = window.ko.contextFor(node); } catch { continue; }
      const d = ctx?.$data;
  ...
  function imageTrendMeta(el) {
    ...
    for (const n of chain) {
      let ctx;
      try { ctx = window.ko?.contextFor?.(n); } catch {}
      const d = ctx?.$data;
      if (!d) continue;
      for (const k of keys) {
  ...
  async function envelope({ captureType, captureMethod, sourceId, payload, provenance = {}, notes = [] }) {
    ...
    const header = {
      ...
      relationships,
      provenance: {
        source: 'dom', adapter: isImageTrend ? 'imagetrend' : 'generic-dom', derived: false, ...provenance
      },
      page: pageMeta(),
      device: deviceMeta(),
      exportHints: {
        suggestedFilename: `${baseName}-${datePart}.json`,
        chatFilename: `${baseName}-${datePart}-chat.txt`,
        mimeType: 'application/json',
        chatMimeType: 'text/plain',
        reportSection: captureType
      }
    };
    header.hash = await digestObject({ header: { ...header, flags: undefined, relationships: undefined }, payload });
  ...
  async function inspect(el, eventType = null, captureMethod = 'element-pick') {
    ...
    const payload = {
      ...
      ancestors: (() => {
        const out = [];
        let n = el;
        for (let i = 0; n && i < 8; i++, n = n.parentElement) out.push(genericElement(n, eventType));
        return out;
      })()
    };
    return envelope({
      captureType: 'element', captureMethod,
      sourceId: sourceIdFor(el, imageTrend), payload,
  ...
  async function pageScan() {
    ...
    return envelope({
      captureType: 'page-scan', captureMethod: 'page-scan',
      sourceId: location.pathname, payload, provenance: { scope: 'page' },
      notes: [
        'Read-only page scan.',
        'The export contains DOM observations and metadata only; it does not write to the inspected page.',
        'Input values are intentionally excluded.'
      ]
    });
  }
  ...
  function chatBundle(capture) {
    return {
      format: CHAT_EXPORT_VERSION,
      purpose: 'Chat-friendly GFI capture. Machine-readable JSON stored in a plain UTF-8 text file.',
      capture
    };
  }
  ...
  function downloadText(text, filename) {
    ...
    a.href = url;
    a.download = filename || `gfi-capture-${Date.now()}.txt`;
  ...
  function exportCaptureAsChatFile(capture) {
    if (!capture?.header?.captureId) return false;
    const filename = capture.header.exportHints?.chatFilename || `${slug(capture.header.friendlyName || capture.header.captureType || 'gfi-capture')}-chat.txt`;
    downloadText(JSON.stringify(chatBundle(capture), null, 2), filename);
    return true;
  }
  ...
  shadow.innerHTML = `<style>
    :host{font:12px system-ui;color:#e6edf3}button,a,input{font:inherit}.launch{width:38px;height:38px;border-radius:999px;border:1px solid #2dd4bf;background:#111827;color:#d1fae5;font-weight:800;box-shadow:0 4px 14px #0007}.panel{width:390px;max-width:90vw;max-height:76vh;overflow:auto;background:#0f1720;border:1px solid #2dd4bf;border-radius:10px;padding:9px;box-shadow:0 10px 28px #0008}.head{display:flex;justify-content:space-between;align-items:center}.hide{border:0;background:transparent;color:#e6edf3;font-size:18px}.row{display:flex;gap:4px;flex-wrap:wrap;margin:5px 0}.row button,.row a{padding:5px 7px;color:#e6edf3;background:#17212b;border:1px solid #2dd4bf;border-radius:7px;text-decoration:none;cursor:pointer}.meta{border-top:1px solid #334155;border-bottom:1px solid #334155;margin:7px 0;padding:7px 0}.meta label{display:block;color:#94a3b8;font-size:10px;margin:3px 0}.meta input{box-sizing:border-box;width:100%;padding:5px 6px;background:#09111a;color:#e6edf3;border:1px solid #334155;border-radius:6px}.status{font-size:11px;margin:5px 0;color:#99f6e4}.summary,.helpbody{color:#94a3b8;font-size:11px}.power,.export{border-top:1px solid #334155;margin-top:7px;padding-top:7px}.help{border-top:1px solid #334155;margin-top:7px;padding-top:7px}.help summary{cursor:pointer;color:#d1fae5;font-weight:700}.helpbody b{color:#cbd5e1}pre{white-space:pre-wrap;word-break:break-word;background:#09111a;color:#cbd5e1;border:1px solid #334155;padding:6px;border-radius:6px;max-height:200px;overflow:auto;font-size:10px}pre[hidden]{display:none}</style>
    ...
    <div class="panel" hidden>
      ...
      <div class="summary"><b>READ ONLY.</b> GFI observes the DOM and exports observations. It does not submit forms, invoke page actions, or write values into the inspected site.</div>
      <div class="meta">
      ...
      <div class="export"><strong>Export</strong>
        <div class="row"><button id="export-page">Export page .txt</button><button id="export-current">Export current .txt</button></div>
        <div class="summary">The .txt file contains clean, pretty-printed JSON so it can be uploaded directly into chat without pasting a giant block.</div>
      </div>
      ...
      <details class="help"><summary>Help / what these buttons do</summary><div class="helpbody">
        <p><b>Pick element:</b> arms one read-only capture. Your next tap/click is intercepted so the underlying page action does not fire.</p>
        <p><b>Last/focused:</b> captures the most recently touched or focused element without activating it.</p>
        <p><b>Page scan:</b> inventories candidate controls, bindings, text, geometry, and supported framework metadata visible in the current DOM.</p>
        <p><b>Export page .txt:</b> runs a fresh page scan and downloads a UTF-8 text file containing structured JSON for direct chat upload.</p>
        <p><b>Export current .txt:</b> downloads whatever capture is currently loaded.</p>
        <p><b>Copy JSON / header:</b> puts the full current capture or only its normalized metadata header on the clipboard.</p>
        <p><b>Clear:</b> clears the current in-memory capture only. Saved friendly-name/tag/profile preferences remain.</p>
        <p><b>Parent:</b> links later captures to the current capture ID for comparison/reconstruction.</p>
        <p><b>Copy selector / JS probe:</b> copies diagnostic text only; GFI does not execute the probe.</p>
        <p><b>Read-only boundary:</b> GFI may store its own preferences locally and create export files, but it does not write into site fields or call site actions.</p>
      </div></details>
  ...
  async function captureEvent(e) {
    ...
    if (!pickMode || sameGesture(e)) return;
    pickMode = null;
    e.preventDefault?.();
    ...
    saveMetaInputs();
    setTimeout(async () => {
      render(await inspect(target, e.type, 'element-pick'));
      status(`Captured ${target?.tagName || 'element'} via ${e.type}. Underlying action blocked.`);
    }, 0);
  }
  ...
  document.addEventListener('focusin', e => {
    if (!e.composedPath?.().includes(host) && e.target instanceof Element) lastTarget = e.target;
  }, true);

  $('.launch').onclick = () => { $('.panel').hidden = false; $('.launch').hidden = true; };
  $('.hide').onclick = () => { $('.panel').hidden = true; $('.launch').hidden = false; };
  $('#pick').onclick = () => { pickMode = 'element'; status('Tap/click the element to inspect. That gesture will be blocked from the page.'); };
  $('#inspect').onclick = async () => {
    const el = lastTarget || document.activeElement;
    if (!(el instanceof Element) || el === document.body) return status('Tap or focus an element first.');
    saveMetaInputs(); render(await inspect(el, 'manual', 'last-focused')); status('Captured last/focused element.');
  };
  $('#scan').onclick = async () => {
    saveMetaInputs(); render(await pageScan()); status(`Page scan captured ${result?.data?.elements?.length || 0} candidates.`);
  };
  $('#export-page').onclick = async () => {
    saveMetaInputs();
    const capture = await pageScan();
    render(capture);
    exportCaptureAsChatFile(capture);
    status(`Page scan exported as ${capture.header.exportHints.chatFilename}. Upload that .txt directly into chat.`);
  };
  $('#export-current').onclick = () => {
    if (!exportCaptureAsChatFile(result)) return status('Capture something first.');
    status(`Current capture exported as ${result.header.exportHints?.chatFilename || 'text file'}.`);
  };
  ...
  $('#pin-parent').onclick = () => {
    if (!result?.header?.captureId) return status('Capture something first.');
    parentCaptureId = result.header.captureId; status(`Parent pinned: ${parentCaptureId}`);
  };
  $('#clear-parent').onclick = () => { parentCaptureId = null; status('Parent link cleared.'); };
  $('#copy-selector').onclick = async () => {
    const el = lastTarget;
    if (!(el instanceof Element)) return status('Inspect an element first.');
    status(await copyText(selectorFor(el) || '') ? 'Selector copied.' : 'Copy failed.');
  };
  $('#copy-probe').onclick = async () => {
    const el = lastTarget;
    if (!(el instanceof Element)) return status('Inspect an element first.');
    status(await copyText(consoleProbe(el)) ? 'Console probe copied. GFI did not execute it.' : 'Copy failed.');
  };
})();