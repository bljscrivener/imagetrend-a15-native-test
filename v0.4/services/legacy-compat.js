(() => {
  ...
  const service = Object.freeze({
    ...
    clinicalAvailable(){ return !!window.GremlinA15Clinical?.prepareSalineFlush; },
    ...
    async prepareSalineFlush(route,doseMl){
      const clinical=window.GremlinA15Clinical;
      if(!clinical?.prepareSalineFlush) throw new Error('A15 clinical saline-flush helper unavailable');
      return clinical.prepareSalineFlush({ route, doseMl:Number(doseMl), confirm:true });
    },
    subscribe(fn){
      const b=bridge();
  ...
  core.registerWorker({
    id:'saline-flush-finger', version:VERSION, resources:['chart-mutation','clinical-medication'],
    async run(job,ctx){
      const legacy=ctx.service('legacy-a15');
      const route=String(job.payload?.route || '').toUpperCase();
      const doseMl=Number(job.payload?.doseMl);
      if(!['IV','IO'].includes(route)) throw new Error('saline flush route must be IV or IO');
      if(!Number.isFinite(doseMl) || doseMl <= 0 || doseMl > 100) throw new Error('saline flush dose must be >0 and <=100 mL');
      ctx.progress(.1,'saline-flush-precheck');
      const result=await legacy.prepareSalineFlush(route,doseMl);
      if(!result?.ok) throw new Error(result?.reason || 'saline flush preparation failed');
      ctx.progress(1,'saline-flush-prepared');
      return result;
    }
  });
  ...
  const api=Object.freeze({
    ...
    prepareSalineFlush(route,doseMl){
      if(!service.clinicalAvailable()) throw new Error('A15 clinical saline-flush helper unavailable');
      return core.submit({worker:'saline-flush-finger',pipelineId:'clinical-active',stage:'saline-flush',priority:5,payload:{route,doseMl}});
    },
    state:()=>service.state(),
    available:()=>service.available(),
    clinicalAvailable:()=>service.clinicalAvailable()
  });

  Object.defineProperty(window,'GremlinA15LegacyCompat04',{value:api,configurable:false,writable:false});
  core.emit('legacy-compat-ready',{version:VERSION,available:service.available(),clinicalAvailable:service.clinicalAvailable()});
})();
