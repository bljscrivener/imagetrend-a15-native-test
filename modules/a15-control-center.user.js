...
(() => {
  ...
  function currentFindings() {
    try { return window.GremlinA15Findings?.getFindings?.() || []; } catch { return []; }
  }
  ...
  function mount() {
    ...
    shadow.innerHTML = `
      <style>
        :host{font:13px system-ui,-apple-system,sans-serif;color:#e5e7eb}
        ...
        .view[hidden]{display:none}.row{display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin:7px 0}.stack{display:grid;gap:6px}
        .muted{color:#94a3b8;font-size:12px;line-height:1.35}.status{font-size:12px;color:#99f6e4;min-height:16px;margin-top:6px}
        ...
        label.toggle{display:flex;gap:7px;align-items:flex-start}.box{border:1px solid #334155;border-radius:8px;padding:8px;background:#0f172a}.danger{border-color:#7f1d1d}.help{line-height:1.42;color:#cbd5e1;font-size:12px}.help strong{color:#f8fafc}
      ...
      <section class="panel" hidden>
        ...
        <div class="view" data-view="settings" hidden>
          ...
          <div class="box">
            <label class="toggle"><input type="checkbox" id="reconstruction"><span><strong>Adaptive reconstruction</strong><br><span class="muted">Recommended. Stores control/action metadata only so A15 can recover from ImageTrend UI changes. It does not record keystrokes or persist patient-entered scalar values.</span></span></label>
          </div>
        ...
        <div class="view help" data-view="help" hidden>
          ...
          <div class="box"><strong>Native AI</strong><p>A15's AI bridge only invokes ImageTrend's own AI controls after an explicit user action. It does not forward chart content to a separate AI service.</p></div>
  ...
  function renderFindings() {
    ...
    if (!rows.length) {
      root.innerHTML = '<div class="muted">No current A15 findings. Run checks after the chart section you want to evaluate is visible.</div>';
      return;
    }
  ...
  function wire() {
    ...
    $('#rescan').onclick = () => { const r = window.GremlinA15Reconstruction?.scan?.('manual-ui'); status(r?.fingerprint ? `Map refreshed: ${r.fields?.length || 0} fields, ${r.actions?.length || 0} actions.` : 'Mapper unavailable or disabled.'); };
  ...
  waitForRuntime().then(r => {
    ...
    runtime.registerModule({
      id: MODULE_ID,
      version: VERSION,
      description: 'Compact A15 control center and help/settings surface.',
      defaultEnabled: true,
      start: async () => mount(),
      stop: async () => { document.getElementById(HOST_ID)?.remove?.(); shadow = null; }
    });
