...
(() => {
  ...
  function expandChoice(c){
    ...
    return entries.flatMap((entry,i)=>{if(path.includes('.PatientProcedures[]')&&!procedureName(entry,{quiet:true}))return [];return [{...c,entry,label:c.label,entryLabel:(path.includes('.Vitals[]')?'Vital':'Procedure')+' '+(i+1)}];});
  }
