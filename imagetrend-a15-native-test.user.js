...
(() => {
  ...
  function captureLocal(root,reason){
    ...
    const payload=JSON.stringify({schema:1,version:'0.2.4.17',reason,url:location.href,capturedAt:new Date().toISOString(),incident:data});
  ...
  const about=document.createElement('small');about.textContent='Gremlin Logic A15 0.2.4.17 · Updates are managed by Tampermonkey. Observation is limited to field IDs and order in this tab.';settings.append(about);
  ...
  const copyLog=document.createElement('button');copyLog.id='copy-diagnostic-log';copyLog.textContent='Copy diagnostic log';copyLog.onclick=()=>copyReport({version:'0.2.4.17',events:diagnostics,issueCount:$('#issues').children.length,issues:Array.from($('#issues').children,row=>({field:row.querySelector('button')?.textContent?.replace(' — Open field',''),reason:diagnosticReason(row.querySelector('small')?.textContent||'')}))});toolsPanel.insertBefore(copyLog,footer);
  let lastField=null;
